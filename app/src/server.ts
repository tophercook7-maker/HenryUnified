...
app.post("/chat", (req, res) => {
  const q = (req.body && (req.body.query || (req.body.messages && req.body.messages[0] && req.body.messages[0].content))) || "";
  req.body = { query: q };
  // fall through to same logic as /ask by reusing handler below if structured that way
  // or re-run the same internal function your /ask handler uses here if it is inline
});
  try {
